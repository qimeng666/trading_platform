...
// simple trade application for trade storage and viewing.
//
// Authors:
//  YOUR NAME
...
const tradeapp_db = require('./tradeapp_db.js')
const { HeadBucketCommand, ListObjectsV2Command } = require('@aws-sdk/client-s3');
const { tradeapp_s3, s3_bucket_name, s3_region_name } = require('./tradeapp_s3.js');
...
app.listen(config.service_port, () => {
  ...
  //
  process.env.AWS_SHARED_CREDENTIALS_FILE = config.tradeapp_config;
});
...
app.get('/', (req, res) => {
  try {
    ...
    res.json({
      ...
      "dbConnection": tradeapp_db.state
    });
  }
  catch(err) {
